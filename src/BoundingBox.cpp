...
BoundingBox::BoundingBox(const Point3 & c, const float extent_x, const float extent_y, const float extent_z) {
	...
	this->h_y = Vector3(0.0, (extent_x / 2.0) + 0.1, 0.0); //this vector will detect a floor collision before a wall collision
	this->h_z = Vector3(0.0, 0.0, extent_z / 2.0);
	this->h_y2 = Vector3(0.0, extent_y / 2.0, 0.0);
}
...
bool BoundingBox::collidesWith(const BoundingBox & b) { //floor collision
	if (between(projectOntoAxis(*this, X), projectOntoAxis(b, X))
		&& between(projectOntoAxis(*this, Y2), projectOntoAxis(b, Y))
		&& between(projectOntoAxis(*this, Z), projectOntoAxis(b, Z)))
	{ return true; }
	return false;
}

bool BoundingBox::collidesWithWall(const BoundingBox & b) {
	if (between(projectOntoAxis(*this, X), projectOntoAxis(b, X))
		&& between(projectOntoAxis(*this, Y2), projectOntoAxis(b, Y2))
		&& between(projectOntoAxis(*this, Z), projectOntoAxis(b, Z)))
	{ return true; }
	return false;
}
...
pair<float, float> BoundingBox::projectOntoAxis(const BoundingBox & b, enum AXIS axis) {
	...
	case Z:
		...
		break;
	case Y2:
		lo = projection(Point3(Vector3(*(b.c)) + (-1 * b.h_y2)), UNIT_Y_AXIS);
		hi = projection(Point3(Vector3(*(b.c)) + b.h_y2), UNIT_Y_AXIS);
		break;
	}
